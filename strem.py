...
st.set_page_config(page_title="Парсер Реестра Недвижимости", layout="wide")
...
def parse_pdf_file(uploaded_file):
    try:
        ...
        if not results:
            return "Нет зарегистрированной недвижимости"
...
st.title("📄 Парсер выписок из Реестра Недвижимости")
st.write("Загрузите один или несколько PDF-файлов для обработки.")
...
if st.button("Обработать файлы"):
    if not uploaded_files:
        ...
        if not global_results:
            formatted_text = "Нет данных для отображения."
        else:
